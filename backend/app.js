...
app.post('/api/cancel-appointment', async (req, res) => {
    try {
        ...
        if (randevuSorgu.rows.length === 0) return res.status(404).json({ success: false, message: "Randevu bulunamadı." });
        ...
        try {
            const response = await ai.models.generateContent({
                ...
                contents: `Bir kuaför dükkanında ${iptalEdilen.randevu_tarihi} tarihinde saat ${iptalEdilen.randevu_saati} koltuğu az önce iptal edilerek boşaldı. Sırada bekleyen diğer müşterilere gönderilmek üzere, onları dükkana davet eden, yapay zeka sıcaklığında, samimi, aciliyet hissi uyandıran Türkçe bir WhatsApp kurtarma mesajı yaz. Mesajda dürüst ve kuaför esnafı dilini kullan, emoji ekle, kibar ol. Sadece gönderilecek mesaj metnini dön.`,
            });
        ...
        res.status(500).json({ success: false, message: "İptal motoru hatası." });
    }
});
