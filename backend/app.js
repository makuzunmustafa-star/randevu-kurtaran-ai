...
// ⭐ API: BEKLEME LİSTESİNE EKLE (Müşteri tarafı, açık erişim)
app.post('/api/waitlist', async (req, res) => {
    try {
        ...
        const siraSorgu = await pool.query(
            'SELECT COALESCE(MAX(sira_no), 0) as max_sira FROM bekleme_listesi WHERE dukkan_slug = $1',
            [temizSlug]
        );
        const yeniSira = parseInt(siraSorgu.rows[0].max_sira, 10) + 1;

        const eklenen = await pool.query(
            'INSERT INTO bekleme_listesi (dukkan_slug, musteri_adi, musteri_telefon, tercih_edilen_hizmet, sira_no) VALUES ($1, $2, $3, $4, $5) RETURNING id',
            [temizSlug, musteriAdi, musteriTelefon, tercihEdilenHizmet || null, yeniSira]
        );

        res.json({ success: true, message: "✅ Bekleme listesine eklendiniz! Bir boşluk açılırsa SMS ile haber vereceğiz.", id: eklenen.rows[0].id, siraNo: yeniSira });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});

// ⭐ API: BEKLEME LİSTESİNİ GÖRÜNTÜLE (Panel, şifre korumalı)
app.get('/api/waitlist/:slug', panelYetkisiKontrolEt, async (req, res) => {
    try {
        const dukkanSlug = req.dukkanSlug;
        const liste = await pool.query(
            'SELECT id, musteri_adi, musteri_telefon, tercih_edilen_hizmet, sira_no, created_at FROM bekleme_listesi WHERE dukkan_slug = $1 ORDER BY sira_no ASC',
            [dukkanSlug]
        );
        res.json({ success: true, bekleyenler: liste.rows });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});

// ⭐ API: BEKLEME LİSTESİNDEN SİL (Panel, şifre korumalı)
app.delete('/api/waitlist/:slug/:id', panelYetkisiKontrolEt, async (req, res) => {
    try {
        const dukkanSlug = req.dukkanSlug;
        const { id } = req.params;

        const silinecek = await pool.query('SELECT sira_no FROM bekleme_listesi WHERE id = $1 AND dukkan_slug = $2', [id, dukkanSlug]);
        if (silinecek.rows.length === 0) {
            return res.status(404).json({ success: false, message: "Kayıt bulunamadı." });
        }
        const silinenSira = silinecek.rows[0].sira_no;
        ...
        res.json({ success: true, message: "Kayıt silindi ve sıra güncellendi." });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
